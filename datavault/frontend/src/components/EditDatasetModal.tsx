...
interface Props {
  open: boolean;
  onClose: () => void;
  dataset: { id: string; name: string; description?: string | null; is_bridge?: boolean } | null;
}
...
export default function EditDatasetModal({ open, onClose, dataset }: Props) {
  ...
  const noChanges = trimmed === dataset.name
    && description.trim() === (dataset.description ?? "").trim()
    && isBridge === !!dataset.is_bridge;
  const disabled = !trimmed || noChanges;
    ...
    <div style={{
      ...
      <div style={{
        ...
        <label style={{
          ...
          background: isBridge ? "var(--color-primary-bg)" : "var(--color-bg)",
          border: `1px solid ${isBridge ? "var(--color-primary)" : "var(--color-border)"}`,
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: "var(--color-text)" }}>
              Es una tabla intermedia (N:N)
            </div>
            <div style={{ fontSize: 11.5, color: "var(--color-text-muted)", marginTop: 2, lineHeight: 1.4 }}>
              Marca esta opción si este dataset solo existe para conectar otros dos
              (ej. <code>OperacionInversionista</code> con FK a Operaciones y FK a Inversionistas).
              Quedará oculto de la lista principal por defecto.
            </div>
          </div>
        </label>
